...
GOIMPORTS_CMD = $(TEMP_DIR)/gosimports -local github.com/boss-net
...
unit: $(TEMP_DIR)  ## Run unit tests (with coverage)
	...
	go test -coverprofile $(TEMP_DIR)/unit-coverage-details.txt $(shell go list ./... | grep -v boss-net/clio/test)
